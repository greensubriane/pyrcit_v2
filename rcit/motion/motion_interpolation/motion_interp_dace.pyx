...
"""
Cython module for global motion spatial interpolation with
krigin and krigin local interpolation algorithm
"""
...
def motion_interp_kirgin(np.ndarray[float64, ndim=2] motion):
    ...
    # correlation = corr_cubic
    cdef int mx = motion.shape[0]
    cdef int my = motion.shape[1]
    cdef int nv = motion.shape[0] * motion.shape[1]
    S1 = []
    S2 = []
    Y = []
    n = -1
    for ix in range(mx):
    # for ix in prange(mx, schedule='dynamic', nogil=True):
        for iy in range(my):
...
@cython.wraparound(False)
@cython.cdivision(True)
cdef inline count_nan(np.ndarray[float64, ndim=2] fi):
    # f1 = fi
...
@cython.wraparound(False)
@cython.cdivision(True)
def motion_interp_kriging_local(np.ndarray[float64, ndim=2] motion):
